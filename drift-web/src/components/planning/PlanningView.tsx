...
export function PlanningView({ projectName, userRole, onComplete, onCancel }: PlanningViewProps) {
  ...
  const handleCreateProject = async () => {
    setCreating(true)
    try {
      // Create the project with selected tasks
      const selectedTasks = plans.flatMap(p => 
        p.tasks.filter(t => t.checked).map(t => ({
          role: p.role,
          title: t.title,
          description: t.description,
          priority: t.priority,
      ...
      // TODO: Also create tasks via API if needed
